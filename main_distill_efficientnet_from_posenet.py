...
import os
from models.posenet.PoseNet_for_distill import PoseNet
...
if __name__ == "__main__":
    ...
    # Create the model
    from models.posenet.PoseNet_for_distill import PoseNet, PoseNet_origional, PoseNet_origional_mamba
    from torchvision import models
    feature_extractor = models.resnet34(pretrained=True)
    model_teacher =  PoseNet_origional(feature_extractor, droprate=0.5, pretrained=True,
            filter_nans=False,return_feature=True).to(device)
    # model_teacher = get_model(args.model_name, args.backbone_path, args.use_mamba, config).to(device)
   
    model_student = PoseNet( backbone_path=args.backbone_path,return_feature=args.return_feature).to(device)
    ...
    multiscene = False #model_teacher.classify_scene
    ...
    if args.mode == 'train':
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, minibatch in enumerate(dataloader):
                ...
                with torch.no_grad():
                    res_teacher = model_teacher(minibatch.get('img'))

                est_pose_teacher = res_teacher.get('pose')
                feature_map_teacher = res_teacher.get('feature_map_posenet')#('local_descs_t')#('feature_map_t')
                # print('feature_map_teacher.shape = ',feature_map_teacher.shape) #torch.Size([8, 2048])
                ...
                if args.use_mamba:
                    ...
                    if args.use_classical_mamba:
                        if args.scene in ['OldHospital', 'KingsCollege', 'ShopFacade', 'StMarysChurch']:
                            command = f"python main_distill_efficientnet_from_posenet.py transposenet test ./models/backbones/efficient-net-b0.pth /home/transposenet/data/Cambridge /home/transposenet/datasets123/CambridgeLandmarks/abs_cambridge_pose_sorted.csv_{args.scene}_test.csv --checkpoint_path {weights_filename} --use_mamba True --scene {args.scene} --skipvim {args.skipvim} --use_classical_mamba True" #/home/transposenet/out/{checkpoint_prefix}_checkpoint-{epoch}.pth"
                        else: 
                            command = f"python main_distill_efficientnet_from_posenet.py transposenet test ./models/backbones/efficient-net-b0.pth /home/transposenet/data/7Scenes /home/transposenet/datasets123/7Scenes/abs_7scenes_pose.csv_{args.scene}_test.csv --checkpoint_path {weights_filename} --use_mamba True --scene {args.scene} --skipvim {args.skipvim} --use_classical_mamba True" 
                    else:
                        if args.scene in ['OldHospital', 'KingsCollege', 'ShopFacade', 'StMarysChurch']:
                    ...
                    if args.scene in ['OldHospital', 'KingsCollege', 'ShopFacade', 'StMarysChurch']:
                        ...
                        command = f"python main_distill_efficientnet_from_posenet.py transposenet test ./models/backbones/efficient-net-b0.pth /home/transposenet/data/7Scenes /home/transposenet/datasets123/7Scenes/abs_7scenes_pose.csv_{args.scene}_test.csv --checkpoint_path {weights_filename} --scene {args.scene} --skipvim {args.skipvim}" 
        ...
        if args.use_mamba:
            if args.use_classical_mamba:
                results_filename = f'results_distill_efficientnet_from_posenet_use_classical_mamba_{args.scene}_skip_{args.skipvim}.txt'
            else:
                results_filename = f'results__distill_efficientnet_from_posenet_mamba_{args.scene}_skip_{args.skipvim}.txt'
            
        else:
            results_filename = f'results__distill_efficientnet_from_posenet_{args.scene}_skip_{args.skipvim}.txt'
